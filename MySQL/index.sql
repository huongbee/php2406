...
(20) NOT NULL COMMENT 'mã sản phẩm' , 
    PRIMARY KEY
(`id`), UNIQUE `ma-san-pham`
(`product_index`)
);
ALTER TABLE `products` 
CHANGE `product_index` `product_index` VARCHAR
(30) 
...
( 
    ...
    `name` VARCHAR
(100) NOT NULL , 
    `description` TEXT NULL DEFAULT NULL , PRIMARY KEY
(`id`)
);
...
ALTER TABLE `products`
ADD  CONSTRAINT `FK_PRODUCTS_VS_TYPE` FOREIGN KEY
(`type_id`)
    REFERENCES `type`
(`id`);
-- 1-n  n-1 : FK: n
...
-- FROM table_name
-- [   WHERE condition 
--     GROUP BY list_of_column_groupby
--     HAVING condition_groupby
